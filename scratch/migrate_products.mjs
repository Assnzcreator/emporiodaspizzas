...
const burgers = [
  ...
  // ACOMPANHAMENTOS
  { id: "batata-frita", name: "Batata Frita G", description: "Batatas crocantes com sal e maionese da casa.", price: 18.90, image: "https://images.unsplash.com/photo-1573080496219-bb080dd4f877?w=800&q=80", category: "acompanhamentos" },
...
async function migrate() {
  console.log(`Starting migration of ${burgers.length} items...`);
  
  const formatted = burgers.map(b => ({
    ...b,
    available: true,
  ...
  const { data, error } = await supabase
    .from('products')
    .upsert(formatted, { onConflict: 'name' }) // Using name as conflict since IDs might clash with UUIDs if already present
    .select();

  if (error) {
    console.error('Migration error:', error);
  } else {
    console.log('Migration successful!', data?.length, 'items processed.');
  }
}
